...
- [x] **ST7789 显示驱动与双缓冲渲染 (`components/display_ui`)**
  ...
  - [x] **二级层级菜单系统 (2-Level Sub-Menu)**：
    ...
    - 二级子菜单：详细操作项，支持超过 6 项时的平滑视口滚动及 `< Back to Main Menu` 返回
    - 单键操作体验：短按循环切换光标，长按 >1.5s 蓄力进入子菜单或执行动作
  - [x] **屏幕双向横屏旋转 (0° <-> 180°)**：
    - 在 System Tools 菜单增加 `2. Screen Rotate 180`，一键翻转屏幕，适配不同角度插入 USB 接口的使用习惯
    - Web 管理后台同步支持实时切换屏幕方向
  - [x] **三大模块专用详情信息页 (Info Sub-Tabs)**：
    - **TF Info**：存储容量/用量/挂载模式 (Page 1) + 活跃会话/文件名/刷盘参数 (Page 2)
    - **Wi-Fi Info**：STA 连接状态/SSID/Host IP/Gateway (Page 1) + AP 热点/信道/MAC 地址 (Page 2)
    - **System Info**：ESP32-S3 CPU/Free Heap/Min Heap/Uptime (Page 1) + 固件版本/Flash/屏幕方向 (Page 2)
    - **极简交互**：详情页内单击短按快速切页/翻页，长按 >1.5s 蓄力条平滑返回上一级菜单
  - [x] 动态蓄力进度条：长按时在底栏绘制 `Hold OK: xx% [====>   ]` / `Hold Back: xx%`
  - [x] 执行弹窗提示 (Applying)：长按确认后弹出专用提示窗口（包括 TF 格式化与屏幕旋转专属弹窗）
  - [x] 菜单 12 秒超时无操作自动返回仪表盘
- [x] **实机编译与烧录验证**
